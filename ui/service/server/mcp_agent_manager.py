...
class MCPAgentManager(ADKHostManager):
    ...
    def is_mcp_agent_query(self, message: Message) -> tuple[bool, str]:
        ...
        elif any(word in query for word in ['flight', 'air', 'plane', 'voo']):
            return True, 'Air Ticketing Agent'
        elif any(word in query for word in ['hotel', 'accommodation', 'stay']):
            return True, 'Hotel Booking Agent'
        elif any(word in query for word in ['car', 'rental', 'vehicle', 'carro']):
            return True, 'Car Rental Agent'
